...
void main(void)
{
    ...
    vec2 vFontSize = vec2(40.0, 80.0);
  	vec4 vColour = vec4(0.7);
    vColour = mix( vColour, vec4(1.0, 1.0, 0.0, 0.0), PrintValue(gl_FragCoord.xy, vec2(0.0, 10.0), vFontSize, RENDERSIZE.x, 2.0, 2.0));
    vColour = mix( vColour, vec4(0.7, 0.0, 0.5, 0.0), PrintValue(gl_FragCoord.xy, vec2(150.0, 10.0), vFontSize, RENDERSIZE.y, 2.0, 2.0));
    vColour = mix( vColour, vec4(0.7, 0.0, 0.5, 0.0), PrintValue(gl_FragCoord.xy, vec2(250.0, 10.0), vFontSize, TIME, 2.0, 0.0));
